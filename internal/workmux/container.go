...
func (c *Containers) checkEngineImage(ctx context.Context, engine sandboxEngine, config SandboxConfig) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("inspect sandbox image %q in the configured Podman image store (build or pull it explicitly): %w", config.Image, err)
	}
	var images []struct {
		ID string `json:"Id"`
	}
...
func (c *Containers) runArgs(ctx context.Context, engine sandboxEngine, w Workspace, command string, env []string, interactive, input bool) ([]string, error) {
	...
	return append(args, image, "sh", "-c", command), nil
}
...
func (c *Containers) localEngine(ctx context.Context) (sandboxEngine, error) {
	...
	if err != nil {
		return engine, fmt.Errorf("identify legacy Podman store: %w", err)
	}
	var info struct {
		Host  struct{ DatabaseBackend string }
		Store struct {
	...
	if err := json.Unmarshal(out, &info); err != nil {
		return engine, fmt.Errorf("decode legacy Podman store: %w", err)
	}
