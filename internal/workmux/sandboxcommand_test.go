...
func TestSandboxCommandParser(t *testing.T) {
	for _, test := range []struct {
		...
		{args: []string{"sandbox", "shell", "-e"}},
		{args: []string{"sandbox", "shell", "--exec"}},
		...
		t.Run(strings.Join(test.args, " "), func(t *testing.T) {
			...
			if test.valid && (got.Help != test.help || !reflect.DeepEqual(got.ShellCommand, test.words)) {
...
func TestSandboxShellMainAndSubdirectory(t *testing.T) {
	c, w, _ := sandboxTestFixture(t)
	app := sandboxCommandApp(c, w)
	...
	for _, dir := range []string{w.Root, subdir} {
		...
		if err := app.Run(t.Context(), Command{Kind: "sandbox", Name: "shell"}); err == nil || !strings.Contains(err.Error(), "no registered running sandbox") {
...
func TestSandboxRunPlainWorktree(t *testing.T) {
	...
	if err := app.Run(t.Context(), Command{Kind: "sandbox", Name: "run", ShellCommand: []string{"bash"}}); err != nil {
		t.Fatal(err)
	}
	if !run {
