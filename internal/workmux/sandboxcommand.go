...
func parseSandboxCommand(args []string) (Command, error) {
	...
	usage := func() (Command, error) {
		return Command{}, fmt.Errorf("usage: cli workmux init [-g|--global] | sandbox build | sandbox run -- command... | sandbox shell [-- command...]")
	}
	args = args[1:]
	if c.Kind == "sandbox" {
		if len(args) == 0 {
		...
		if c.Name != "build" && c.Name != "shell" && c.Name != "run" {
			return usage()
		}
	}
	for i, arg := range args {
		if arg == "--" {
			if c.Name != "shell" && c.Name != "run" && i+1 != len(args) {
				return usage()
			}
...
func (app *App) runSandboxCommand(ctx context.Context, command Command) (result error) {
	...
	return c.Shell(ctx, state.Workspace, true, command.ShellCommand, app.Stdin, app.Stdout, app.Stderr)
}
...
func (c *Containers) RunStandalone(ctx context.Context, w Workspace, words []string, stdin io.Reader, stdout, stderr io.Writer, visible func() error) error {
...
func (c *Containers) shell(ctx context.Context, w Workspace, existing bool, words []string, stdin io.Reader, stdout, stderr io.Writer, visible func() error) (result error) {
	...
	if existing {
		...
		var registration func(bool) error
		if w.Standalone {
		...
		defer func() {
			...
			if registration != nil {
				result = errors.Join(result, registration(err == nil))
			}
		}()
		if w.Standalone && visible != nil {
