...
func TestSandboxCommandsIntegration(t *testing.T) {
	...
	output.Reset()
	stderr.Reset()
	// Run and shell retain -it. The container PTY merges its stdout and
	// stderr; these are not two independent child streams when redirected.
	if err := app.Run(ctx, Command{Kind: "sandbox", Name: "run", ShellCommand: []string{"printf 'shell-ok\\n'; printf 'child-stderr-token\\n' >&2; printf persist > shell-file"}}); err != nil {
	...
	if found, err := c.ownedSessions(ctx, w, true); err != nil || len(found) != 0 {
		t.Fatalf("standalone session survived: %v %v", found, err)
	}
	argv, err := c.PaneCommand(ctx, w, "sleep 300")
	if err != nil {
		t.Fatal(err)
	}
	// Only the long-lived fixture is detached. Run and shell above/below
	...
	output.Reset()
	if err := app.Run(ctx, Command{Kind: "sandbox", Name: "shell", ShellCommand: []string{"printf exec-ok"}}); err != nil {
		t.Fatalf("exec: %v\n%s", err, &stderr)
	}
	if !strings.Contains(output.String(), "exec-ok") {
		t.Fatal(output.String())
	}
	err = app.Run(ctx, Command{Kind: "sandbox", Name: "shell", ShellCommand: []string{"exit 23"}})
