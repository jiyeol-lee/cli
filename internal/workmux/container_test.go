...
func TestContainersInspectRegistrationRace(t *testing.T) {
	for _, mode := range []string{"appeared", "disappeared again", "inspection failure", "listing failure", "malformed", "wrong name"} {
		t.Run(mode, func(t *testing.T) {
			const name = "cli-workmux-inspection-race"
			failure := fmt.Errorf("inspection unavailable")
			inspections, listings := 0, 0
			c := &Containers{Runner: sandboxCommandRunner(func(_ context.Context, p Process) ([]byte, error) {
				call := sandboxTestPodmanProcess(p)
				if call.Args[0] != "container" {
					t.Fatalf("unexpected process: %+v", call)
				}
				switch call.Args[1] {
				case "inspect":
					inspections++
					if inspections == 1 || mode == "inspection failure" || mode == "disappeared again" {
						return nil, failure
					}
					if mode == "malformed" {
					...
					return json.Marshal([]map[string]string{{"Id": strings.Repeat("a", 64), "Name": foundName}})
				case "ls":
					listings++
					if mode == "listing failure" {
						return nil, failure
					}
					if mode == "disappeared again" && listings == 2 {
						return nil, nil
					}
					return []byte(name + "\n"), nil
				default:
			...
			found, err := c.inspect(t.Context(), sandboxCurrentEngine(), name)
			switch mode {
			case "appeared":
				if err != nil || found == nil || found.Name != name {
					t.Fatalf("new registration = %+v, %v", found, err)
				}
			case "disappeared again":
				if err != nil || found != nil {
					t.Fatalf("confirmed absence = %+v, %v", found, err)
				}
			default:
				if err == nil || found != nil {
					t.Fatalf("accepted uninspected registration: %+v, %v", found, err)
				}
				if (mode == "inspection failure" || mode == "listing failure") && !errors.Is(err, failure) {
					t.Fatalf("lost engine failure: %v", err)
				}
			}
			wantInspections, wantListings := 2, 1
			if mode == "listing failure" {
				wantInspections = 1
			}
			if mode == "inspection failure" || mode == "disappeared again" {
				wantListings = 2
			}
			if inspections != wantInspections || listings != wantListings {
				t.Fatalf("unbounded or missing retry: %d inspections, %d listings", inspections, listings)
			}
		})
	}
}
...
func TestContainersEphemeralPane(t *testing.T) {
	...
	for _, argv := range [][]string{first, second} {
		...
		if argv[0] != "/usr/bin/env" || process.Name != "podman" || !slices.Equal(process.Args[:3], []string{"run", "--rm", "-it"}) || !slices.Equal(argv[len(argv)-4:], []string{engine.image, "bash", "-c", command}) {
	...
	for _, call := range sandboxEngineCalls(engine) {
		if call.Args[0] != "image" && call.Args[0] != "info" {
...
func TestContainersCommandUsesBash(t *testing.T) {
	for _, interactive := range []bool{false, true} {
		t.Run(fmt.Sprint(interactive), func(t *testing.T) {
			c, w, _ := sandboxTestFixture(t)
			command := `values=('quoted value' retained); [[ ${#values[@]} == 2 ]] || exit 1; printf '%s:%s' "${values[0]}" "${values[1]}"; exit 7`
			args, err := c.runArgs(t.Context(), sandboxCurrentEngine(), w, command, nil, interactive, false)
			if err != nil {
				t.Fatal(err)
			}
			argv := args[len(args)-3:]
			if !slices.Equal(argv, []string{"bash", "-c", command}) {
				t.Fatalf("sandbox interpreter = %q", argv)
			}
			// Execute only the command suffix locally, without starting Podman.
			out, err := exec.CommandContext(t.Context(), argv[0], argv[1:]...).Output()
			var status *exec.ExitError
			if !errors.As(err, &status) || status.ExitCode() != 7 || string(out) != "quoted value:retained" {
				t.Fatalf("Bash command = %q, %v", out, err)
			}
		})
	}
}

func TestContainersSharedRelabel(t *testing.T) {
	for _, interactive := range []bool{false, true} {
		t.Run(fmt.Sprint(interactive), func(t *testing.T) {
			c, w, _ := sandboxTestFixture(t)
			args, err := c.runArgs(t.Context(), sandboxCurrentEngine(), w, "true", nil, interactive, false)
			if err != nil {
				t.Fatal(err)
			}
			mounts, readonly := 0, 0
			for i, arg := range args {
				if arg == "--security-opt" || strings.Contains(arg, "label=disable") || strings.Contains(arg, "workmux_container_t") {
					t.Fatalf("custom labeling option: %q", args)
				}
				if arg != "--mount" {
					continue
				}
				mounts++
				value := args[i+1]
				if !strings.HasPrefix(value, "type=bind,") || strings.Count(value, "relabel=") != 1 || !slices.Contains(strings.Split(value, ","), "relabel=shared") {
					t.Fatalf("not exactly shared: %s", value)
				}
				if slices.Contains(strings.Split(value, ","), "readonly") {
					readonly++
				}
				for _, forbidden := range []string{":Z", ":U", "relabel=private"} {
					if strings.Contains(value, forbidden) {
						t.Fatalf("unsafe mount: %s", value)
					}
				}
			}
			if mounts == 0 || readonly == 0 || mounts == readonly {
				t.Fatalf("missing writable or protected mounts: %q", args)
			}
		})
	}
}
...
func TestContainersExecFinite(t *testing.T) {
	...
	if !slices.Equal(last.Args[:3], []string{"run", "--rm", "-i"}) || !slices.Equal(last.Args[len(last.Args)-3:], []string{"bash", "-c", "exit 7"}) || last.Stdin != stdin || last.Stdout != &stdout || last.Stderr != &stderr || stdout.String() != "input" {
...
func TestContainersPaneRestoresCallerEngineInTmux(t *testing.T) {
	...
	args := []string{"new-window", "-d", "-P", "-F", "#{pane_id}", "-t", session + ":", "-c", w.Path, "--", "/bin/bash", "-c", nativeShellCommand(argv, "/bin/bash")}
