...
func TestSandboxSignalProcess(t *testing.T) {
	...
	if mode != "podman" {
		os.Args = []string{"cli", "workmux", "sandbox", "run", "--", "bash"}
		if mode == "exec" {
	...
	for _, received := range []syscall.Signal{syscall.SIGINT, syscall.SIGTERM} {
		for _, mode := range []string{"fresh", "exec", "cleanup"} {
			t.Run(fmt.Sprintf("%s/%s", mode, received), func(t *testing.T) {
				...
				assertStandalone := func(active bool) {
					t.Helper()
					want := 0
					if active {
						want = 1
					}
					for _, pattern := range []string{"standalone/*/*.json", "containers/*/runs/*"} {
						paths, err := filepath.Glob(filepath.Join(stateBase, pattern))
						if err != nil || len(paths) != want {
							t.Fatalf("standalone %s: got %v, want %d entries: %v", pattern, paths, want, err)
						}
					}
					if paths, err := filepath.Glob(filepath.Join(stateBase, "*", "*.json")); err != nil || len(paths) != 0 {
						t.Fatalf("standalone run created managed workspace state: %v, %v", paths, err)
					}
					if active {
						data, err := os.ReadFile(filepath.Join(base, "container.json"))
						if err != nil {
						...
						if err := json.Unmarshal(data, &containers); err != nil || len(containers) != 1 {
							t.Fatalf("standalone container: %s, %v", data, err)
						}
						if containers[0].Config.Labels["io.github.jiyeol-lee.cli.workmux.standalone"] != "true" {
							t.Fatal("run lacks standalone ownership label")
						}
					}
				}
				...
				for {
					...
					select {
					case <-ctx.Done():
						t.Fatal("standalone run did not release repository lock")
					case <-time.After(5 * time.Millisecond):
					}
				}
				assertStandalone(true)
