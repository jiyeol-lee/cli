...
cli workmux sandbox run -- command...
cli workmux sandbox shell [-- command...]
cli workmux add <branch> [--base <ref>] [-l|--layout <name>] [-b|--background]
...
`add --open-if-exists` opens an existing branch worktree. `--dry-run` reports the plan without provisioning. The three add skip flags suppress lifecycle hooks, file operations, or pane commands for that invocation. `open` focuses an existing window or creates one using current configuration. `--new` creates another window; `--force-files` reapplies file setup and `--run-hooks` reruns `post_create`. `close` closes the owned window and stops managed sandbox sessions without deleting the worktree or branch. It leaves standalone `sandbox run` sessions alone. Use `--` to end options and `cli workmux --help` for usage.
...
After saving configuration from another pane, close and reopen the affected workspace panes to launch fresh containers. Fresh panes, `sandbox run`, and finite sandbox commands all use shared relabeling. `sandbox shell` enters an existing container without relabeling or recreating it. Image builds are unchanged; rebuilding the image is not needed for this change.
...
Build the image explicitly. Workmux uses Podman only and respects its native connection and storage environment, including remote connection settings. It does not force `--remote=false`. Add, open, run, and shell never build or pull a missing image. Use the engine/image store that will run your panes.
...
Run a standalone sandbox from the main checkout or any linked Git worktree, including subdirectories and worktrees not previously managed by Workmux. These commands need no tmux session, do not create worktrees or run hooks, and ignore `sandbox.enabled`:

```sh
cli workmux sandbox run -- bash
cli workmux sandbox run -- git status --short
cli workmux sandbox shell
cli workmux sandbox shell -- 'printf "%s\\n" "$PWD"'
```

`sandbox run` requires `-- command...` and starts an independent `podman run --rm -it` session using the same protected Git and OpenCode mounts as sandboxed panes. Arguments after `--` are joined with spaces and executed by `bash -c`, not passed as a literal argv. Quote shell syntax accordingly. Concurrent sessions have separate containers but share checkout files. Exit or cancellation removes that session's container, not persistent bind-mounted files. The CLI forwards SIGINT and SIGTERM to its foreground Podman client and returns 130 or 143 respectively. Command exit statuses are preserved.

Run and shell always use `-it`, even when redirected, without TTY auto-detection. Container stdout and stderr share a PTY and normally arrive together on the Podman client's stdout; terminal line endings and formatting can differ from pipe output. The separately injected stderr receives Podman client diagnostics, not a guaranteed separate container stderr stream. These commands are not a byte-preserving noninteractive pipe interface.

Standalone runs use separate session records, not managed workspace history. Workmux creates and locks the session record under the repository lock, then releases the repository lock before launching Podman. The session record stays locked through launch, execution, and cleanup, blocking destructive worktree removal and merge cleanup even before the container starts. Interactive sessions do not retain the repository lock. `close` excludes standalone sessions. Finished standalone runs do not give host-only workspaces a Podman dependency. Managed pane launches retain sandbox history for recovery, and legacy mount snapshots or endpoint records still trigger safety checks.

`sandbox shell` only enters an existing registered running container with `podman exec -it`. It defaults to Bash; optional `-- command...` uses the same shell-string semantics as run. The former `-e` and `--exec` flags are rejected. With several matches, selection is deterministic and the CLI names the selected container and the others. Registration uses ownership labels for this repository and checkout, including the endpoint recorded at launch; it does not select by image or focused pane. Missing, stopped, mismatched, and pre-endpoint sessions produce errors rather than launching a replacement or entering unrelated containers. Shell does not inspect or build the image, and exit or cancellation does not remove the original container. Legacy persistent containers remain subject to the migration restrictions below.
